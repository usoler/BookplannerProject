...
nLibros = random.randint(1, 15)#minimo un libro
...
for i in range(nLibros):
    ...
    if p <= 25: #le intentamos dar un predecesor
        ...
        while (predecesor == i or predecesor in dag[i] or len(dag[predecesor])>0) and countTimes < 25:
            predecesor = random.randint(0, nLibros-1)
            countTimes += 1
...
pddlText =  "(define (problem test) (:domain ext1_planner)\n" 
...
os.makedirs(os.path.dirname("./JuegosDePruebaGenerados/ext1_problem_generated.pddl"), exist_ok=True)
with open("./JuegosDePruebaGenerados/ext1_problem_generated.pddl", "w") as juegoDePrueba:
...
try:
    os.system("../../Metric-FF/ff -o ../Extension_1/ext1_domain.pddl -f ./JuegosDePruebaGenerados/ext1_problem_generated.pddl")
except:
